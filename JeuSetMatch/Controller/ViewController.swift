...
class ViewController: UIViewController {
    ...
    private func updateAllSetsScore(forPlayer player: Player) {
        for sets in 0..<5 {
            if sets < match.sets.count {
                let set = match.sets[sets]
                playerSetScoreLabels[player]![sets].text = "\(set.scores[player]!)"
            } else {
                playerSetScoreLabels[player]![sets].text = ""
            }
        }
    }

    private func updateGameScore(forPlayer player: Player) {
        if let advantagedPlayer = match.currentGame.advantagedPlayer, advantagedPlayer == player {
                playerGameScoreLabels[player]!.text = "Av."
        } else {
